...
__global__ void global_memory_sum(float * d_out, float * d_in)
{
...
int main( int argc, char* argv[] ) {
  ...
  */
  ...
  for(int i = 0 ; i < NUM_TRIALS; i++){
    ...
    global_memory_sum<<<num_blocks, num_threads>>>(d_intermediate, d_in);

    // KERNEL CALL 2ND TIME
    num_threads = num_blocks; // launch one thread for each block in prev step
    num_blocks = 1;
    global_memory_sum<<<num_blocks, num_threads>>>(d_out, d_intermediate);
  }
  ...
  cout << "Memory Allocation " << array_size*sizeof(float) <<endl;
